...
Then('the user can see language and theme in parameters section', async function () {
    await this.expect(this.page.locator('#parameters-section >> text=language|langue|theme|thème')).toBeVisible(); 
});

Then('the user can see a change password button', async function () {
    await this.expect(this.page.locator('#change-password-button')).toBeVisible(); 
});
...
Then('the language changes', async function () {
    await this.expect(this.page.locator('html')).toHaveAttribute('lang', /en|fr|de|es/i);
});
...
Then('the user is redirected to profile modification page', async function () {
    await this.expect(this.page).toHaveURL(/.*edit|modification|profile.*edit.*/i);
});
...
Then('the form should contain a field for the old password', async function () {
    await this.expect(this.page.locator('#old-password')).toBeVisible(); 
});

Then('the form should contain a field for the new password', async function () {
    await this.expect(this.page.locator('#new-password')).toBeVisible(); 
});

Then('the form should contain a field for confirming the new password', async function () {
    await this.expect(this.page.locator('#confirm-password')).toBeVisible(); 
});